...
function hasRealMoves(movesText) {
  // Also treat null move (--) as real content
...
function buildTree(movesText) {
  ...
  function parse(tokens, parentNode, moveNum, isBlack) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (looksLikeSan(tok) || tok === '--') {
        const san = tok === '--' ? '--' : tok.replace(/[!?]+$/, '');
        const leftover = tok === '--' ? '' : tok.slice(san.length);
...
function serialize(node, inVariation = false, forceNumber = false) {
  if (!node.san) {
    // Root node
  ...
  // Show move number when:
  // - It's a white move (always)
  // - It's a black move AND we're in a variation (need context)
  // - It's a black move AND forceNumber is set (after comment/variation in mainline)
  const showNumber = !node.isBlack || inVariation || forceNumber;
  ...
  // After a comment or sideline variation, the next black move needs its number shown
  const nextNeedsNum = (node.comments.length > 0 || vars.length > 0) && main.isBlack;

  out += ' ' + serialize(main, false, nextNeedsNum);
