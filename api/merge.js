...
function buildTree(movesText) {
  ...
  function parse(tokens, parentNode, moveNum, isBlack) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      // Move number token — extract actual number and side
      if (/^\d+\./.test(tok)) {
        const num = parseInt(tok);
        const dots = tok.replace(/^\d+/, '');
        curMoveNum = num;
        curIsBlack = dots.length >= 3; // "..." means black to move
        continue;
      }
      ...
      if (looksLikeSan(tok)) {
        const san = tok.replace(/[!?]+$/, '');
        // Save branch point BEFORE advancing
        branchPoint = cur;
        branchMoveNum = curMoveNum;
        branchIsBlack = curIsBlack;
...
function serialize(node, inVariation) {
  if (!node.san) {
    // Root node
    if (node.children.length === 0) return '';
    let out = '';
  ...
  // Always show move number for white moves
  // Show move number for black moves only when starting a variation or after a comment/variation break
  if (!node.isBlack) {
    out += `${node.moveNum}. `;
  } else if (inVariation) {
    out += `${node.moveNum}... `;
  ...
  // After a comment or variation, black needs move number shown
  const mainNeedsNum = node.comments.length > 0 || vars.length > 0;
  out += ' ' + serialize(main, mainNeedsNum && main.isBlack ? true : false);
